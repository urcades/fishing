# Fishing Protocol — draft 0.1

Status: a working, versioned draft for native ports. Schema revision `1` is independent of the crate's `0.1.0` release version. Do not claim stable-v1 compatibility yet. The canonical implementation is Rust; the independent Python port and Rust/WASM build run the same conformance corpus.
...
Within struggle: rest → warning → surge → rest
```

Initial state: tick/elapsed timers/durations 0; rest behavior; P=.25, T=0, E=1; primary=0; no reason; RNG=provided seed. Tracking starts fish/tackle/targets at .5, velocities and steer at 0.
...
4. Waiting: when incremented phaseTicks reaches duration, enter bite with phaseTicks=0 and its resolved duration; emit `bite`. A press on this transition does not hook. The player must release and press again.
5. Bite: test expiry before the press. If incremented phaseTicks >= duration, escape with `missed_bite`. Otherwise a fresh press emits `hooked`, then either `caught` (hook mode) or enters rest and emits `rest` (struggle modes). Thus a state with phaseTicks=duration−1 is already too late to hook on its next step.
...
9. If no earlier terminal return and total tick >=3600, escape with `timeout`. This overrides nonterminal events from that tick. Catch takes precedence over timeout.

Events are ordered strings: `cast`, `bite`, `hooked`, `rest`, `warning`, `surge`, `caught`, `escaped`. At most two per call. Terminal input is absorbing; reset is a new `create_state` call, not another transition.
...
- Duration conversion is `max(1, floor(seconds × 60 + 0.5))`; **not Python's ties-to-even round**.
...
`Definition` supplies mechanisms, base parameters, capture geometry and hookBonus in [0,.5]. `Loadout` supplies complete numeric fish/rod/bait profiles. Metadata, names and IDs stay outside the protocol. An active profile field replaces its matching base parameter:
...
- Fish bite duration and bait apply to all modes. Resolve waitMin=`1.5/(attraction*affinity)` capped [.4,5], waitMax=`3/(attraction*affinity)` capped [.4,8], biteWindow=`fish.biteWindow + definition.hookBonus + bait.biteBonus` capped [.5,2]. Report each cap in notes.
